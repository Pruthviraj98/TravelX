...
//4396 2034 8293 0204 VISA WORKS
if(!isset($_SESSION['login']))
{
	header('Location: logout.php');
}
	if(isset($_POST['invoice_btn']))
	{
		...
		{
		$disc_code = $_POST['discount'];
		$sqlq = mysqli_query($db, "SELECT DISCOUNT_CODE from discount where DISCOUNT_CODE = '$disc_code'");
		echo '$sqlq';
		if(mysqli_num_rows($sqlq))
		{
			$sql = "SELECT DISCOUNT from discount where DISCOUNT_CODE = '$disc_code'" ;
...
<html>
<head>
<title>Payment - Train Reservation</title>
...
bodd h2,h3,h4,h5,h6 {font-family: "Raleway", Arial, Helvetica, sans-serif}
/*-- main --*/
.mainw3-agile{
    padding: 3em 0 0;
}
...
<body>
	...
	<center><a href="#" class="w3-button w3-hover-red"><b><i class="fa fa-map-marker w3-margin-right"></i><h1>Travelx Payment Portal</h1></b></a></center>
		<?php
	...
	*/?>
		...
		<div class="main-agileinfo">
			<div class="w3pay wthree-pay-grid">
				<div class="card-bounding agileits">
					<form action="payment.php" method="post">
						<h2>
						<?php

							$date3 = $_SESSION['t_date'];
							$name = $_SESSION['name'];

							$as = $_SESSION['a_s'];
							$ts = $_SESSION['t_s'] ;
							list($year,$month,$day)=explode("-", $date3);
							$year = $year%100;
							...
							$sql="SELECT TRAIN_NAME FROM train WHERE TRAIN_CODE='$code'";

							$train=mysqli_query($db,$sql);
							if(!$train){echo mysqli_error($db);}
							$row = mysqli_fetch_assoc($train);
							$_SESSION['t_name'] = $row['TRAIN_NAME'];
							...
							if($s1 == 1)
							{
								$sqlq = "SELECT DEPT from train where TRAIN_CODE = '$code'";
								$result=mysqli_query($db,$sqlq);
								$row1 = mysqli_fetch_assoc($result);
								$s1 = $row1['DEPT'];
								$s1 = ucwords(strtolower($s1));
							}

							if($s2 == 1)
							{
								$sqlq = "SELECT ARR from train where TRAIN_CODE = '$code'";
								$result=mysqli_query($db,$sqlq);
								$row1 = mysqli_fetch_assoc($result);
								$s2 = $row1['ARR'];
								$s2 = ucwords(strtolower($s2));
							}

							echo "<aside> {$row['TRAIN_NAME']} Going from $s1 to $s2 </aside><br>
							<aside>Date of Travel:  </aside>
							<div class='card-details'>
								  <input value = '$date3'>
							</div>
								  ";
						    $sql2 = "SELECT COST FROM train WHERE TRAIN_CODE='$code'";
							$cost=mysqli_query($db,$sql2);
							if(!$train){echo mysqli_error($db);}
							$row = mysqli_fetch_assoc($cost);
							$_SESSION['t_cost'] = $row['COST'];

						if(!(isset($SESSION['iserror'])))
						{
							$_SESSION['disc_code2'] = "DEMO20";
							$disc_code = $_SESSION['disc_code2'];
						}
						else
						{
							$_SESSION['disc_code2'] = "INVALID CODE";
							$disc_code = $_SESSION['disc_code2'];
						}
						echo "
						<div class='card-details '>
						...
						<aside>
						<div class='left'><h2> Cost of Purchase :</h2>
						<input value = '{$row['COST']}' > 	</div>
						<div class='right'><h2>Discount Code: (optional)</h2>
						<div class = 'card-container'>
						<input type='text' name= 'discount'	placeholder='$disc_code' > </div><div class='card-valid'><i class='fa fa-check' aria-hidden='true'></i></div></div>
						</aside>
						...
						<br>
						<div class='card-details'>
							<aside><h2>Card Holder's Name:</h2></aside> ";
						?>
						<?php	echo "<input type='text' name='name' placeholder= '$name' required=''/>  </div>"; ?>
						<aside><h3>Aadhar Number: </h3></aside>
						<input type="text" name="aadhar" placeholder="0000 0000 0000" maxlength="14" onKeyUp="$cc.validate(event)" required=''>
						<br>
						<br><br>
						...
						<div class="card-container">
							...
							<input type="text" name="cardno" placeholder="0000 0000 0000 0000"  maxlength="19" onkeyup="$cc.validate(event)" required=""/>
						...
						<div class="card-details agileits-w3layouts">
							<div class="expiration">
								...
								<input type="text" name="exp" onkeyup="$cc.expiry.call(this,event)" maxlength="7" placeholder="mm/yyyy" required=""/>
							</div>
							<div class="cvv">
								<aside>CVV</aside>
						...
						<input type="submit" name ="invoice_btn" value="Pay Now">
					</form>
				</div>
				<text align = "center">
				<h6><a href="schedule_.php"><i class="fa fa-angle-double-left" aria-hidden="true"></i> Cancel your payment</a></h6>
				</text>
			</div>
